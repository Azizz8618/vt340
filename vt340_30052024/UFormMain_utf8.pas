...
var
Version      // Версия
             :  string = '(Версия 02 от 17.05.2024)';
...
//* Изделие ТЕСТ                                           *
//*--------------------------------------------------------*
//* Компонент: Менеджер памяти                             *
//*--------------------------------------------------------*
//* Основной модуль                                        *
//*--------------------------------------------------------*
//* Автор:     http://fastmm.sourceforge.net               *
//* Создание:  20.03.2007                                  *
//* Изменение: $Date:: 2009-04-14 22:40:44 +0400#$         *
//**********************************************************
//
// $Id: FastMM4.pas 502 2009-04-14 18:40:44Z olekhov $
//
...
//* Компонент: Менеджер памяти                             *
//*--------------------------------------------------------*
//* Модуль с текстовыми сообщениями                        *
//*--------------------------------------------------------*
//* Автор:     http://fastmm.sourceforge.net               *
//* Создание:  20.03.2007                                  *
//* Изменение: $Date:: 2009-04-14 22:40:44 +0400#$         *
//**********************************************************
//
// $Id: FastMM4Messages.pas 502 2009-04-14 18:40:44Z olekhov $
//
...
// Добавление Ctrl-E, Ctrl-J в конец строки
// ========================================
// Эта функция предназначена только для копирования и вставки.
function AddCtrlEJ
 (Str   // Входная строка
        :  string)
 // Выходная строка
...
begin
NumSym:=AnsiPosNum(CR+LF, Str, 1);
while NumSym <> 0 do  //Теперь данная функция только для режима BLINK
// if (TypeShowCtrl in [0,2]) or (ButtonBLINK.Tag = 0) and (NumSym <> 0) then
begin
 if AddCtrlE = false
  then
   Insert(' j', Str, NumSym)
...
LineSepIns:=FileSave.ReadBool('Screen', 'LineSepIns', false);
//LineSepSym:=chr(FileSave.ReadInteger('Screen', 'LineSepSym', ord('e')));
LineSepSym:=chr(FileSave.ReadInteger('Screen', 'LineSepSym', ord('^')));
LineSepNum:=FileSave.ReadInteger('Screen', 'LineSepNum', 20);
Memo.Color:=FileSave.ReadInteger('Screen', 'ColorScr', clWhite);
Memo.Font.Charset:=FileSave.ReadInteger('Screen', 'FontCharset', RUSSIAN_CHARSET);
Memo.Font.Color:=FileSave.ReadInteger('Screen', 'FontColor', clGreen);
...
NumKey // Номер клавиши
       :  integer;
begin
//if KeyIn = VK_RETURN then
//begin Result:='c'; Exit end;
if KeyIn = VK_RETURN then
begin Result:=#03; Exit end;
case KeyIn of
$C0: KeyIn:=ord('`');
$BD: KeyIn:=ord('-');
$BB: KeyIn:=ord('=');
...
if (TypeShowCtrl in [0,2]) or (ButtonBLINK.Tag = 0) then
 for NumSym:=1 to Length(Str) do
  if Str[NumSym] in ['a'..'z'] then Str[NumSym]:=' ';
//NumFirstLine:=Memo.Perform(EM_GETFIRSTVISIBLELINE, 0, 0);
NumFirstLine:=GetFirstVisibleLine(Memo);
//
Memo.Text:=Str;
//
Memo.SelStart:=PosScr;
//LineScroll(Memo, NumFirstLine); // ВРЕДНО!!! НЕ ИСП.
//
//
//Memo.Perform(EM_LINESCROLL, 0, NumFirstLine);
...
if SetRaw = true then
 ...
 IdTelnet.SendString(#$07+'set raw'+CR+LF+CR+LF);
 Sleep(1500);
 IdTelnet.SendString(StrToBESM6('НУС'#03));
 end;
ButtonONClick(Self);
end;
//
...
Str:=Clipboard.AsText;
Str:=AnsiUpperCase(Str);
Str:=AddCtrlEJ(Str); // Нужно переделывать
Insert(Str, StrScreen, Memo.SelStart+1);
StrScreenToMemo(false);
end;
//
...
0:
 ...
 ButtonROLL.Font.Style:=[fsBold];
 ButtonROLL.Tag:=1;
// Конфликт с уже имеющейся процедурой
(*  i := 15 * (memo.Lines.Count div 15);
  while i > 14 do begin
    memo.Lines.Delete(pred(i));
    dec(i, 15);
...
if Key in ['a'..'z'] then // Ctrl
 begin
 Result:=chr(ord(Key)-ord('a')+1); // Проверить правильность формулы перевода
 Exit;
 end;
if Key in ['А'..'Я'] then // Русские буквы
 begin
...
if ord(Key) in [$01..$1A] then
 Result:=chr(ord(Key)+$60);
if Result = 'm' then       // Возврат каретки = ETX
 Result:='c';
end;
//
// Перевод принятого символа с клавиатуры в символ НВМЗ
...
NumKey // Номер клавиши
       :  integer;
// Нажатие клавиши "Ввод" на клавиатуре
if KeyIn = VK_RETURN then
 begin Result:='c'; Exit end;
case KeyIn of
$C0: KeyIn:=ord('`');
...
//
// Перепись копии экрана в Memo - Основная процедура выдачи
// ============================
//Проблема мерцания связана с ошибками в процедуре прокрутки
//экрана Мемо.
//
